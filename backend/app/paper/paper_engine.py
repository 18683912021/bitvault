...
- 现货模型：USDT 现金 + 币持仓；买入扣现金（含手续费），卖出减持仓（手续费从现金扣）。
...
class PaperEngine:
    ...
    def reset(self, initial: float = 10000.0) -> dict:
        acct = {"usdt": initial, "coins": {}, "initial": initial}
        self._save(acct)
    ...
    async def place_intent(self, intent: dict) -> dict:
        ...
        if ins["instType"] != "SPOT":
            raise RiskBlocked("本地模拟盘当前仅支持现货（SPOT）标的")
        ...
        order_id = db.execute(
            ...
            " px, sz, state, source, venue, created_at, updated_at)"
            " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            (_new_cl_ord_id(), instance_id, inst_id, "cash", side, "", send_ord_type,
             px, sz_base, "live", source, "paper", now, now),
        )
        db.add_audit(source, "paper_place", {"instId": inst_id, "side": side,
                                             "ordType": send_ord_type, "px": px, "sz": sz_base}, "ok")
